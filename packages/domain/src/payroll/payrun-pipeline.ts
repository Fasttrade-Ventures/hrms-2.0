...
export function computeEmployeePayrun(input: EmployeePayrunInput): EmployeePayrunResult {
  ...
  const socso = lookupSocsoContribution(socsoWage, socsoCategory);

  const eisWage = socsoWageBase.gt(0) ? socsoWageBase : statutoryBase;
  const eisEmp = eisEmployee(eisWage, input.eisEligible);
  const eisEr = eisEmployer(eisWage, input.eisEligible);
