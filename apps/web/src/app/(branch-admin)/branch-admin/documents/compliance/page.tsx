...
function buildComplianceHref(page: number) {
  if (page <= 1) return "/branch-admin/documents/compliance";
  return `/branch-admin/documents/compliance?page=${page}`;
}

export default async function BranchDocumentCompliancePage({
  searchParams,
}: {
  ...
  const [allRows, requiredTypes] = await Promise.all([
    buildComplianceMatrix({ branchIds: context.branchIds }),
  ...
  return (
    <div className="space-y-6">
      <PortalPageHeader
        actions={
          <HrLinkButton href="/branch-admin/documents" variant="outline">
            Back to documents
          </HrLinkButton>
        }
        description={`${context.branchName} · required document status for branch employees`}
