...
import {
  approveRequestAsHr,
  rejectRequestAsHr,
  type HrOperationsActionState,
} from "@/app/(hr)/hr/operations/actions";
...
const initialState: HrOperationsActionState = {};

export function HrApprovalActions({ stepId }: { stepId: string }) {
  const [approveState, approveAction, approvePending] = useActionState(approveRequestAsHr, initialState);
  const [rejectState, rejectAction, rejectPending] = useActionState(rejectRequestAsHr, initialState);
  ...
  return (
    <PortalSectionCard
      description="HR can action any pending org request. This is recorded in the audit log."
      title="HR decision"
    >
